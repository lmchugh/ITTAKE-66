...
LEFT JOIN
		...
		JOIN 
			...
			SELECT
				listingid,
				inventorydate,
				MAX(snapshotdate) AS maxsnapshot
			FROM DW.dbo.VW_InventoryWeeklyFact
			WHERE inventorydate BETWEEN '2015-01-01' AND '2015-12-31'
			GROUP BY 
				listingid,
				inventorydate
			) m ON iwf.listingid = m.listingid AND iwf.inventorydate = m.inventorydate AND iwf.snapshotdate = m.maxsnapshot
		GROUP BY iwf.listingid
		) i ON i.listingid = l.listingid
WHERE	
		1 = 1
		AND l.city = 'Seattle'
